...
- the IR inliner, inlining function where possible, to improve performances by reducing the number of function calls
...
Then, it continues to resolve imports recursively, calling the parser on each new unimported package it finds. The final AST is the agglomeration of all ASTs, where every `import` directive is replaced by a Namespace node, which holds the file's AST, its name, path, and import attributes.

## Macro processor

Once the AST has been built, the macro processor kicks into action and goes over the AST, trying to find any macro node to register them and apply them. Macros are registered by block, so if we are at a current depth of 0 in the AST, and found a macro, it will be available in every node. If we find a macro in a node at depth 2, the macro will get registered and applied in all the potential sub-nodes of the current node, but once we leave the depth 2 node, the macro will be removed from memory and won't be accessible in the other part of the AST.
...
- the executors:
    ...
    - the list one applies predefined macros and user defined macros (function macros), it is arguably the most complex one
...
- if none the previous cases matched, then we most likely have a function call (`ASTLowerer::handleCalls`)
    - we first check if the function call is a short-circuiting operator, `and` or `or`. If it is, we push the first value, and add the corresponding `SHORTCIRCUIT_<AND|OR>` instruction. The next value is added, and it's not the last one, we add the shortcircuit instruction again and keep iterating through the values passed to the operator.
    - otherwise, if the function call isn't an operator, we check if the call is terminal and a self-call (calling the function we are currently compiling). If it is, we replace the call by a jump to address 0 (relative to the current page), transforming the tail call into a loop. If it isn't, we compile the function call to a temporary page.
...
The **IR inliner** is given IR blocks by the **ASTLowerer**, as well as the symbol and value tables. Then, it tries to find matching symbols for each page, to help in the inlining process.

The next thing to do is to go over IR block, try to find any function call and see if it can be inlined, considering some factors like the current IR entities count (since pages have a limit of 65'536 instructions), the function kind (closure, mutating arguments, recursive...), and the argument count.

If an IR block can be inlined at a given call site, it can either end up:
- being replaced by a `CALL_BUILTIN_WITHOUT_RETURN_ADDRESS` instruction,
- being copied with some minor tweaks at the call site, wrapped in a temporary scope.

## IR Optimizer

The **IR Optimizer** is given the IR blocks generated by the **IR inliner**, as well as the symbol and value tables. From there, it visits each page and see if it can merge 2 or more instructions together.

From a list of rules it can apply, it checks if the following instructions match each rule, and if one matches, the rule is executed and generates a new instruction to replace N instructions.

If no rules matched, no instructions were merged. Otherwise, the group of instructions is skipped over and the new instruction is kept in the final IR. The value table is particularly useful when compacting code like `(+ i 2)` to check if the value used for incrementing (or decrementing) is between 0 and 4095 (the maximum value we can encode on 12 bits in super instructions).
...
- Then we compute the list of distinct filenames that were used to generate the IR entities, and write the files table. The instruction location table is constructed by iterating over each IR block, skipping the labels. We keep only one pair of `(page pointer, file, line)` in the resulting table, to avoid storing redondant data in it (as we would only match the first entry of the table, based on the instruction pointer).
